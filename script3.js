...
function generateBox(x, y, z, width, depth, falls) {
  ...
  return {
    threejs: mesh,  // Ensure this property is always present
    cannonjs: body,
    width,
    depth
  };
...
function missedTheSpot() {
  ...
  world.removeBody(topLayer.cannonjs);  // Correct method to remove body
...
function animation(time) {
  if (lastTime) {
    ...
    const topLayer = stack[stack.length - 1];
    if (!topLayer) return; // Ensure topLayer exists
